...
void calculate_thread(Square& canvas, int size, int center_x, int center_y, int scale, int k_value){
    ...
    context.run([&](graphic::GraphicContext *context [[maybe_unused]], SDL_Window *) {
        {
            ...
            ImGui::ColorEdit4("Color", &col.x);
            {
                ...
                for (int iter=0;iter<iter_total;iter++){
                    ...
                    // assign how many threads used in this iter
                    int threads_used=0;
                    if (iter==iter_total-1 && iter_offset>0){
                        threads_used=iter_offset;
                    }else{
                        threads_used=num_threads;
                    }

                    for (int i=0; i<threads_used; i++){
                        thread_arr[i] = std::thread(calculate_thread, std::ref(canvas),size,center_x,center_y,scale,k_value);
                    }
                    for (int i=0; i<threads_used; i++){
                        thread_arr[i].join();
                    }
